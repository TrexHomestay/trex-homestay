...
const PROJECTS = {
  RF_PRINCESS: { en: "R&F Princess Cove", zh: "富力公主湾", total: 45, icon: "🌊" },
  RF_SEINE:    { en: "R&F Seine Region",  zh: "富力新天地",  total: 22, icon: "🏙️" },
};
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          {Object.entries(PROJECTS).map(([k, v]) => (
            <div key={k} style={{ background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: "10px 20px", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)" }}>
...
function Carousel({ images }) {
  ...
  return (
    <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", background: "#000", marginBottom: 24, userSelect: "none" }}>
      <img src={images[idx]} alt="" style={{ width: "100%", height: 360, objectFit: "cover", display: "block" }} />
      {images.length > 1 && <>
        <button onClick={() => setIdx((idx-1+images.length)%images.length)} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.88)", border: "none", borderRadius: "50%", width: 38, height: 38, fontSize: 20, cursor: "pointer", fontWeight: 700 }}>‹</button>
        <button onClick={() => setIdx((idx+1)%images.length)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.88)", border: "none", borderRadius: "50%", width: 38, height: 38, fontSize: 20, cursor: "pointer", fontWeight: 700 }}>›</button>
        <div style={{ position: "absolute", bottom: 10, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 5 }}>
          {images.map((_, i) => <button key={i} onClick={() => setIdx(i)} style={{ width: i===idx?22:7, height: 7, borderRadius: 4, border: "none", background: i===idx?"#fff":"rgba(255,255,255,0.45)", cursor: "pointer", padding: 0, transition: "all .2s" }} />)}
        </div>
        <div style={{ position: "absolute", bottom: 10, right: 14, background: "rgba(0,0,0,0.45)", color: "#fff", borderRadius: 20, padding: "2px 9px", fontSize: 11 }}>{idx+1}/{images.length}</div>
      </>}
    </div>
  );
}
...
function ReviewSection({ p, lang, tx, onAddReview }) {
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      {!p.reviews?.length ? (
        <p style={{ color: "#aaa", fontSize: 14 }}>{tx.no_reviews}</p>
      ) : (
          ...
          {[...p.reviews].reverse().map((r, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: 12, padding: "14px 16px", border: "1px solid #eee", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#0f2d5a", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 15 }}>
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 20px 48px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        ...
        <span style={{ background: p.status === "available" ? "#dcfce7" : "#fee2e2", color: p.status === "available" ? "#16a34a" : "#dc2626", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 700 }}>
          {p.status === "available" ? tx.available : tx.booked}
        </span>
      </div>
...
function AdminLogin({ tx, onLogin }) {
  ...
  return (
    <div style={{ maxWidth: 360, margin: "80px auto", padding: 32, background: "#fff", borderRadius: 20, boxShadow: "0 4px 32px rgba(0,0,0,0.1)", textAlign: "center" }}>
      ...
      <button onClick={() => pw === ADMIN_PW ? onLogin() : setErr(tx.wrong_pw)}
        style={{ width: "100%", padding: "12px", borderRadius: 10, border: "none", background: "#0f2d5a", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>{tx.login_btn}</button>
    </div>
  );
}
...
function AdminPanel({ tx, lang, properties, onUpdate, onLogout }) {
  ...
  const startEdit = (p) => { setForm({ ...p, amenities_en_str: p.amenities_en.join(", "), amenities_zh_str: p.amenities_zh.join(", ") }); setEditId(p.id); setAdding(false); };
...
export default function App() {
  ...
  const filtered = properties.filter(p => {
    const projMatch = project === "ALL" || p.project === project;
